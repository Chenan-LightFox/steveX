...
SteveXAgent (单个 Agent 实例)
    ├── Bot (mineflayer)        ← Minecraft 连接
    ├── Commands (77 个命令)     ← 动作 + 查询 + 创造模式
    ├── LLM (DeepSeekClient)     ← 尚未集成
    ├── Memory                   ← 空文件夹
    ├── Blackboard               ← 空文件夹
    ├── Capsules                 ← 空文件夹
    ├── Git                      ← 空文件夹
...
| 子系统 | 状态 | 说明 |
|--------|------|------|
| **命令系统 (Commands)** | ✅ 完成 | 77 个命令，自动发现加载，分 action / query / creative 三类 |
| **Bot 连接器** | ✅ 完成 | mineflayer 封装，支持连接 Minecraft 服务器 |
| **Agent 基础类** | ✅ 基础 | 启停、命令执行、事件注册 |
| **Agent 管理器** | ✅ 基础 | 多 Agent 启停管理 |
| **Web 面板** | ✅ 基础 | 卡片式 UI，支持 start/stop/chat/command |
| **DeepSeek Client** | ✅ 完成 | LLM 调用客户端，带超时和错误处理 |
| **配置系统** | ✅ 完成 | JSON 配置 + 环境变量覆盖 |
| **日志系统** | ✅ 基础 | console.log 封装 |
...
| 子系统 | 状态 | 预期阶段 |
|--------|------|----------|
| **LLM 集成** | ❌ | Phase 1 — 让 Agent 能用 LLM 自主决策 |
...
| 1.10 | 改造 `server.js` + 前端面板，添加 LLM 聊天界面 | 🔜 |
...
| **LLM Engine** | `src/llm/deepseek_client.js` | 统一的 LLM 调用接口（已存在） | 被调用方 |
...
| Node.js ≥18 | 运行时 |
| CommonJS | 模块系统 |
| mineflayer ^4.x | Minecraft 机器人库 |
| mineflayer-pathfinder ^2.x | 寻路插件 |
| vec3 | 3D 向量 |
| minecraft-data | 方块/物品数据 |
| DeepSeek API | LLM 调用 |
